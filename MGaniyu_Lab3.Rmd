...
# They both have a normal distribution, but the male mean's height is greater that female.
```
...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
    distribution?
```{R, eval = FALSE}
# Looking at the histogram, the density doesnt fit the histogram as expected, but there doesnt appeared to be a significance difference.
```
...
# I would make a conclusion that the data is normally distributed, but we have some outliers (fall on line) at the end of the line.
```
...
# We have alots of outliers on female weight which may not be normally distributed.
```
...
```{r, eval=FALSE}
# We check by using the Qplot

qqnorm(fdims$kne.di)
qqline(fdims$kne.di)
hist(fdims$kne.di, main = "", xlab = "Female Knee");

# it appears that is right skewed.
