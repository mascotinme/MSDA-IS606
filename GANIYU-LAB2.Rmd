...
# A streak length of one means that there was a shot and a miss. There are 0 hits and 1 misses in a streak length of 0.
```
...
```{r, eval= FALSE}
# 1) The distribution is right skewed with a range of 0 to  4, a mode of 0 and a median of 1.
# 2)The typical streak length is 0. 
# 3)The longest streak was 4. 
...
shot_outcomes1 <- c("H", "M")
sim_basket <- sample(shot_outcomes1, size = 133, replace = TRUE);
sim_basket <- sample(shot_outcomes1, size = 133, replace = TRUE, prob = c(0.45, 0.55));
table(sim_basket);
```
...
```{r, eval=FALSE}
qplot(data = my_sim_streak, x = length, geom = "histogram", binwidth = 1)
```
...
```{R, eval=FALSE}
# If I were to run the distribution, I would expect the distribution to be similar to the first run, since the shots are independent of each other. The probability of making the shot doesn't change between the two simulations.
```
...
# The distribution of streak lengths are similar for Kobe and simulated shooter. The evident is insufficient, the hot hand model does not fits Kobe's shooting patterns because the distributions are similar, they both have a modes of 0 and distributions are right skewed with similar ranges.
```
