...
```{r}
# SOLUTION :The number of baptisms is increasingly generally with just a few steep drops. Previous to 1660, there is a small portion of increasing baptisms, but also a longer portion of steep decline which means there is an overall trend in the number of baptisms after around 1660  
```
...
# (1) It Shows here that the year included in this data set is between 1940 & 2002. (2) The dimension or data.frame is 63 observations of 3 variable with 'year', 'boys' and 'girls' as the variables.
```
-   How do these counts compare to Arbuthnot's? Are they on a similar scale?
```{r}
# SOLUTION:
# The data analyzed was over 82 years in Arbuthnot, while it was over 63 years in the Present data. The total data from from Arbutnot was in about 10,000s (ten thousands) while it was in 1,200,000s (millions), but  both gives distribution of both boys and girls over years. 
head(present);
str(present);
dim(present);
head(arbuthnot);
...
# Means: The proportion of boys-to-girls decreases as we go from 1940 to 2000. 
...
```{r}
# SOLUTION:
max(present$boys+ present$girls) == '4268326';
