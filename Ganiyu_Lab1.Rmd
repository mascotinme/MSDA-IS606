...
# SOLUTION: We have 20,000 cases in the data set. DISCRETE VARIABLES ARE: 0 CATEGORICAL VARIABLES ARE: GENHLTH, EXERANY, HLTHPLAN,  SMOKE100, GENDER. COUNTINIOUS VARIABLE ARE: HEIGHT, WEIGHT, WTDESIRE, AGE.

View(cdc); str(cdc); summary(cdc$weight);
head(cdc); tail(cdc);
...
summary(cdc$height, cdc$age);
IQR(cdc$height); IQR(cdc$age);

library(MASS);

gender = cdc$gender
gender.freq = table(gender) / nrow(cdc);
gender.freq;

exerany = cdc$exerany;
exerany.freq = table(exerany) / nrow(cdc);
exerany.freq;
...
# SOLUTION: The mosaicplot reveals more male smoked atleast 100 cigarettes than female did. 
...
```{r}
plot(cdc$weight, cdc$wtdesire, main = "Scatter Plot", ylab = "Weight", xlab = "Desire Weight", pch=20); abline(lm(cdc$weight~cdc$wtdesire), col = "blue"); abline(lowess(cdc$weight~cdc$wtdesire), col = "red");
...
# Showing the first 50 (space management).
```
...
```{r}    
# SOLUTION: wdiff is a scale variable, but it is discrete since the weights are integer values, and thus it can only take integer values itself. When wdiff is equal to 0 that means that the person is at their desired weight. If wdiff > 0 (0,1,2,3.....) then their weight is below their desired value, while wdiff < 0 (-1, -2, -3, -4......)implies that their current weight is above their desired value.
...
```{r}
# SOLUTION: Getting the individual sex summary to minimize and possible outliers.
...
mean(cdc$weight);
sd_weight <- var(cdc$weight, y = NULL, na.rm = FALSE);
standard_variation <- sqrt(sd_weight);
standard_variation;
